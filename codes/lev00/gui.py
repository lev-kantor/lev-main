...
#--------frame for reset controls---------
    ...
    def GetSelection(self):
        ...
        if length <=0:
            length=2000
            print("Warning cannot find length - resorting to default of 2000")
        ...
        if text==None:
            ...
            res=str(len(text))
            res1=str(length)
            c= "Required string length: " + res + " " + "\n" + "Defined Length: LENG2=" + res1 + "\n" + "Increase value of LENG2 in real8.f90 of TETR, recompile TETR and run again"
            tkMessageBox.showinfo("Text to show", c)
        else:
...
#                    tkMessageBox.showwarning("Warning","path found")
                    
                    self.rootdir=home_tetr
                    return
                else:
#                    tkMessageBox.showwarning("Warning","path found")
                    print "$HOME_TETR/tetr does not exist 1"
            else:
                print "$HOME_TETR not defined in enviroment"
#                tkMessageBox.showwarning("Warning","not path found")
#                tkMessageBox.showinfo("new warning", str(home_tetr)+"blah")
